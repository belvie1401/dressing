...
export default function StylistProfilePage() {
  ...
  const [tags, setTags] = useState<string[]>(
    (initialProfile.style_tags as string[]) || []
  );
  const [editingTags, setEditingTags] = useState(false);
  const [newTag, setNewTag] = useState('');
  const [services, setServices] = useState<Service[]>(
    (initialProfile.services as Service[]) || []
  );
  const [serviceModal, setServiceModal] = useState<Service | null>(null);
  const [saving, setSaving] = useState(false);
  const [reviews, setReviews] = useState<Review[] | null>(null);
  const [stats, setStats] = useState<StylistPublicStats | null>(null);
  ...
  useEffect(() => {
    ...
    setTags((initialProfile.style_tags as string[]) || []);
    setServices((initialProfile.services as Service[]) || []);
  }, [initialProfile]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [reviewsRes, statsRes] = await Promise.all([
        api.get<Review[]>('/stylists/me/reviews'),
        api.get<StylistPublicStats>('/stylists/me/public-stats'),
      ]);
      if (!mounted) return;
      ...
      setStats(
        statsRes.success && statsRes.data
          ? statsRes.data
          : { looks_created: 0, satisfaction_percent: null, experience_years: null }
      );
    };
    load();
    return () => {
  ...
  return (
    <div className="min-h-screen bg-[#F7F5F2] pb-24">
      ...
      <section className="px-5 mt-6">
        ...
        {tags.length === 0 && !editingTags ? (
          <p className="text-sm text-[#8A8A8A]">
            Aucun tag. Appuyez sur &laquo;&nbsp;Modifier&nbsp;&raquo; pour en ajouter.
          </p>
        ) : (
        ...
        {editingBio ? (
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <textarea
              ...
              rows={4}
              placeholder="Parlez de votre approche, vos inspirations..."
      ...
      <section className="px-5 mt-6">
        ...
        {services.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
            <p className="text-sm text-[#8A8A8A]">
              Aucune prestation. Ajoutez vos services propos&eacute;s.
            </p>
          </div>
        ) : (
      ...
      <section className="px-5 mt-6">
        ...
        {reviews === null ? (
          <div className="h-24 rounded-2xl bg-[#F0EDE8] animate-pulse" />
        ) : reviews.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
            <p className="text-sm text-[#8A8A8A]">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative w-9 h-9 rounded-full overflow-hidden bg-[#EDE5DC]">
                      {r.avatar ? (
                        ...
                        <div className="flex h-full items-center justify-center">
                          <span className="font-semibold text-[#C6A47E]">{r.author.charAt(0)}</span>
                        </div>
                      )}
                    </div>
...
function StatBlock({
  ...
  return (
    <div className="text-center">
      {value === null ? (
        <div className="h-7 w-14 mx-auto rounded bg-[#F0EDE8] animate-pulse" />
      ) : (
        <p className={`font-serif text-2xl ${gold ? 'text-[#C6A47E]' : 'text-[#111111]'}`}>
          {value}
        </p>
      )}
